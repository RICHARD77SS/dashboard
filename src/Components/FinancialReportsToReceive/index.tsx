...
import Tr from '../Tr';
import { Graph, BoxClassification, BoxColor, ResumeBox, ResumeFlex, ResumeBlock, ResumeContent } from './styles';
        ...
        <Table>
          ...
          <Tbody>
            {dataFinancial?.financial.map((financial: any, index: number) => {
              return new Date(financial.date.split('T'[0])).getMonth() + 1 === month && new Date(financial.date.split('T'[0])).getFullYear() === year && financial.revenuesExpenses === true && financial.paidOut === 'Pendente' ?
                <Tr key={index}>
