...
const FinancialReportsToPay = () => {
  ...
  return (
    <Container>
      <ReportsHeader logo='' corporation='Inc name' reportsName='Fluxo de caixa - Despesas Periodo:' />
        ...
        <Table>
          ...
          <Tbody>
            {dataFinancial?.financial.map((financial: any, index: number) => {
              return new Date(financial.date.split('T'[0])).getMonth() + 1 === month && new Date(financial.date.split('T'[0])).getFullYear() === year && financial.revenuesExpenses === false && financial.paidOut === 'Pendente' ?
                <Tr key={index}>
...
export default FinancialReportsToPay;